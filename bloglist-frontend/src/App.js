...
const App = () => {
  ...
  const showMessage = (message, error=false) => {
    ...
    const timeout = setTimeout(() => setNewMessage({ message:null, error:false }), 2000)
  ...
  const handleLogin = async (event) => {
    ...
    } catch (exception) {showMessage('wrong username or password', true)}
  }
  ...
  const handleLike = async (blogId) => {
    ...
    try {
      await blogService.update(blogId, updatedBlogs.find((blog) => blog.id === blogId))
  ...
  const addBlog  = async (blogObject) => {
    ...
    try {
      ...
      showMessage(`A new blog ${returnedBlog.title} by ${returnedBlog.author} added`, false)

    } catch (error) {showMessage('add title and author', true)}
  }
