describe('Blog app', function() {
  ...
  describe('when logged in', function() {
    ...
    it('a new note can be created', function() {
      cy.createBlogU({ title:'a note created by cypress', author:'cypress tester', url:'url.test' })
    ...
    describe('and a blog exists', function () {
      ...
      it('blog can be liked', function() {
        cy.contains('another blog cypress').parent()
          .contains('View').click()
        cy.contains('another blog cypress').parent()
          .contains('like').click()
    ...
    describe('and a multiple blog exists', function () {
      beforeEach( function() {
        cy.createBlogU({ title:'first note', author:'first a', url:'url.test' })
        cy.createBlogU({ title:'second note', author:'second a', url:'url.test' })
      })

      it('blogs in right order', function() {
        cy.contains('first note').parent()
          .contains('View').click()
        cy.contains('first note').parent()
          .contains('like').click()

        cy.contains('first note liked')
          .then( () => {
            cy.contains('second a').parent()
              .contains('View').click(),
            cy.contains('second a').parent()
              .contains('like').click()
          })
        cy.contains('second note liked')
          .then( () => {
            cy.contains('second a').parent()
              .contains('like').click()
          })
        cy.contains('2')
          .then( () => {
            cy.get('.blog')
              .then(blogs => { cy.wrap(blogs[0]).find('.title').should('contain', 'second note') })
          })
      })
    })
  })
